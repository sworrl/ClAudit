...
_Auto-generated by [ClAudit](https://github.com/sworrl/ClAudit) from 108 filed reports · 82 distinct Request IDs · PII-scrubbed._
...
Every block below stopped authorized work on the reporter's own infrastructure. Each **Request ID is server-side-lookup-able** — the original prompt can be inspected to confirm it was benign and in-scope. This is one underlying defect, surfaced across many surfaces; it is filed as a single report so it can be triaged in one place rather than closed piecemeal as 'duplicates'.
